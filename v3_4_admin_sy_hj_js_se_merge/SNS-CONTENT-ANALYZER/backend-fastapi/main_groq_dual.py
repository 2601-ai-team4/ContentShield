...
SORT_BY_RECENT = 1 # Define SORT_BY_RECENT as 1 for recent comments

@app.post("/crawl/youtube")
async def crawl_youtube(request: YouTubeCrawlRequest):
    """
    Download YouTube comments for a given URL
    """
    try:
        url = request.url
        limit = request.limit if request.limit is not None else 50 # Default limit to prevent timeouts
        
        print(f"[*] Crawling YouTube comments for URL: {url} (Limit: {limit if limit > 0 else 'ALL'})")
        
        from youtube_comment_downloader import YoutubeCommentDownloader
        downloader = YoutubeCommentDownloader()
        comments_gen = downloader.get_comments_from_url(url, sort_by=SORT_BY_RECENT)
